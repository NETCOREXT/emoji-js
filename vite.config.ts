...
export type EmojiStatus = ${[...new Set(lsEmoji.map(e => e.status))].map(g => `'${g}'`).join(' | ')}
export type EmojiGroup = ${[...new Set(lsEmoji.map(e => e.group))].map(g => `'${g}'`).join(' | ')}
export type EmojiSubGroup = ${[...new Set(lsEmoji.map(e => e.subgroup))].map(g => `'${g}'`).join(' | ')}
...
export const EMOJI: Emoji[] = ${JSON.stringify(lsEmoji).replaceAll('\'', '\\\'').replaceAll('"', '\'')}`
      ...
      consola.log('✅ emoji.ts has been updated.')
    },
  }
}
